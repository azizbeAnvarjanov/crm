...
import { useLeadSubmit } from "@/hooks/kindergarten/useLeadSubmit";
import Image from "next/image";
import { useKindClasses } from "@/hooks/kindergarten/useKindClasses";

export default function KindergartenForm({ utm }) {
  const { submitLead, loading } = useLeadSubmit();
  const { kindClasses, loading: kindClassesLoading } = useKindClasses();

  const [fio, setFio] = useState("");
  const [phone, setPhone] = useState("+998 ");
  const [kindClassId, setKindClassId] = useState("");

  // Errors
  const [errors, setErrors] = useState({ fio: "", phone: "", kindClassId: "" });

  // Success modal
  const [success, setSuccess] = useState(false);

  const validate = () => {
    const newErrors = { fio: "", phone: "", kindClassId: "" };
    let valid = true;
    ...
    if (!kindClassId.trim()) {
      newErrors.kindClassId = "Iltimos, sinfni tanlang";
      valid = false;
    }
    ...
    <div className="bg-white shadow-md p-6 rounded-xl space-y-6 max-w-md mx-auto my-auto min-w-[500px] border">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 my-auto">
        ...
        <div>
          ...
          <Select
            onValueChange={setKindClassId}
            value={kindClassId}
            disabled={kindClassesLoading}
          >
            <SelectTrigger>
          ...
          {errors.kindClassId && (
            <p className="text-red-500 text-sm">{errors.kindClassId}</p>
          )}
        </div>
