...
import { useLeadSubmit } from "@/hooks/learning-center/useLeadSubmit";
import Image from "next/image";
import { useDirections } from "@/hooks/learning-center/useDirections";

export default function LearnForm({ utm }) {
  const { submitLead, loading } = useLeadSubmit();
  const { directions, loading: directionLoading } = useDirections();
  ...
  // Errors
  const [errors, setErrors] = useState({ fio: "", phone: "", directionId: "" });

  // Success modal
  const [success, setSuccess] = useState(false);

  const validate = () => {
    const newErrors = { fio: "", phone: "", directionId: "" };
    let valid = true;
    ...
    if (!directionId.trim()) {
      newErrors.directionId = "Iltimos, sinfni tanlang";
      valid = false;
    }
    ...
    <div className="bg-white shadow-md p-6 rounded-xl space-y-6 max-w-md mx-auto my-auto min-w-[500px] border">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 my-auto">
        ...
        <div>
          ...
          {errors.directionId && (
            <p className="text-red-500 text-sm">{errors.directionId}</p>
          )}
        </div>
