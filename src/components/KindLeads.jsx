...
export default function KindergartenLeadsComponent() {
  ...
  const fetchRecordings = useCallback(
    async ({ retries = 0, delayMs = 5000 } = {}) => {
      let attempt = 0;
      let found = false;
      do {
        try {
          setRecsLoading(true);
          const res = await fetch("/api/moizvonki/recordings", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ phone }),
          });
          const data = await res.json().catch(() => ({}));
          if (res.ok && data?.ok && Array.isArray(data.recordings)) {
            setRecs(data.recordings);
            found = data.recordings.length > 0;
          } else {
            setRecs([]);
          }
        } catch (_) {
  ...
  useEffect(() => {
    if (openSheet && phone) {
      fetchRecordings({ retries: 0 });
    }
  }, [openSheet, phone, fetchRecordings]);
          ...
          <div className="space-y-6 p-3">
            ...
            <div className="border p-4 rounded-lg space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      alert("📞 Qo‘ng‘iroq yuborildi");
                      // Auto-refresh recordings shortly after the call to allow processing time
                      setTimeout(() => {
                        fetchRecordings({ retries: 2, delayMs: 7000 });
                      }, 8000);
                    } catch (err) {
                      console.error(err);
                      alert("Server xatosi");
                ...
                <Button
                  variant="outline"
                  disabled={recsLoading || !phone}
                  onClick={async () => {
                    fetchRecordings({ retries: 1, delayMs: 5000 });
                  }}
                >
                  {recsLoading ? "⏳ Yuklanmoqda..." : "🎧 Tinglashlar"}
                </Button>
                      ...
                      >
                        <div className="flex flex-col">
                          <span className="font-medium">
                            {it.meta?.date || "Noma'lum sana"}
                          </span>
                          <span className="text-muted-foreground">
                            {it.meta?.from ? `From: ${it.meta.from}` : ""}
                            {it.meta?.to ? `  To: ${it.meta.to}` : ""}
                            {it.meta?.duration
                              ? `  (${it.meta.duration}s)`
                              : ""}
                          </span>
